...
<cfif isDefined('form.fld_commentSubmit')>
	...
	<cfset variables.aErrorMessages = application.commentsService.validateComment(form.fld_commentAuthor,form.fld_commentContent, form.fld_commentSecuritySystem,form.fld_commentSecurityUser) />
	<!---continue form processing if no errors--->
	<cfif arrayIsEmpty(variables.aErrorMessages)>
		<!--- insert the comment in the database --->
		<cfset application.commentsService.addComment(form.fld_commentAuthor,form.fld_commentContent,url.newsID) />
	</cfif>
...
<cfset securityCode = application.utils.generateCaptcha(7)/>
<div id="comments">
...
<cfform id="frm_comment" preservedata="true" >
	<fieldset>
		...
		<dl>
			...
			<cfimage action="captcha" text="#variables.securityCode#" height="50" width="200" difficulty="medium" fonts="Verdana">
